...
def extract_data_from_db(read_id: str, conn: sqlite3.Connection, cursor: sqlite3.Cursor) -> Optional[Dict[str, Any]]:
    ...
    if row:
        columns = ['read_id', 'fast5_filepath', 'block_stride', 'called_events', 'mean_qscore', 
                   'sequence_length', 'duration_template', 'first_sample_template', 
                   'num_events_template', 'moves_table', 'read_fasta', 'read_quality']
...
def collate_bam_fast5_worker(worker_state: Dict[str, Any], fast5_filepath: str) -> None:
    ...
    for read_id in read_ids_list:
        try: 
            ...
            data = extract_data_from_db(read_id, worker_state['db_connection'], worker_state['db_cursor'])
            # Add basecalling information to the FAST5 file
            f5_read.add_analysis(component='basecall_1d', group_name='Basecall_1D_000', 
                                attrs={'model_type': 'flipflop',  
                                        'name': 'ONT Guppy basecalling software.',
                                        'segmentation': 'Segmentation_000',
                                        'time_stamp': 'Unknown',
                                        'version': 'Unknow'}, config=None)
            f5_read.add_analysis_subgroup(group_name='Basecall_1D_000', subgroup_name='BaseCalled_template')
            moves_array = np.frombuffer(data['moves_table'], dtype='uint8')
            f5_read.add_analysis_dataset(group_name='Basecall_1D_000/BaseCalled_template', dataset_name='Move', data=moves_array, attrs=None)
            fastq = '@' + read_id + '\n' + data['read_fasta'] + '\n' + '+' + '\n' + data['read_quality']
            f5_read.add_analysis_dataset(group_name='Basecall_1D_000/BaseCalled_template', dataset_name='Fastq', data=fastq, attrs=None)
            f5_read.add_analysis_subgroup(group_name='Basecall_1D_000/Summary', subgroup_name='basecall_1d_summary', 
                                attrs={'block_stride': data['block_stride'],
                                        ...
                                        'step_prob':0,
                                        'strand_score': 0})
            f5_read.add_analysis(component='segmentation', group_name='Segmentation_000', 
                        attrs={'name': 'ONT Guppy basecalling software.',
                                'time_stamp': 'Unknown',
                                'version': 'Unknown'})
            f5_read.add_analysis_subgroup(group_name='Segmentation_000/Summary', subgroup_name='segmentation', 
                                attrs={'adapter_max': 0,
            ...
            # throw a loguru exception
            logger.error(f"Error while processing read_id {read_id} in file {fast5_filepath}: {e}")
    mf5.close()
...
def collate_bam_fast5(bam_filepath: str, pod5_dir: str, output_dir: str, num_processes: int):
    ...
    build_index_db(fast5_dir=output_dir, output_dir=output_dir, num_processes=num_processes)
    ...
    with WorkerPool(n_jobs=min(num_processes, num_fast_files) , use_worker_state=True) as pool:
        pool.map(collate_bam_fast5_worker, 
                [(fast5_filepath) for fast5_filepath in fast5_filepaths_list], 
