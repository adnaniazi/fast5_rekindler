...
from pod5.tools.pod5_convert_to_fast5 import convert_pod5_to_fast5
from mpire import WorkerPool  # type: ignore
import pod5 as p5
from pathlib import Path
import sqlite3

def get_pod5_filepath(read_id: str, db_cursor: sqlite3.Cursor) -> str:
...
def make_pod5_file_list(input_dir: str, output_dir:str) -> Tuple[Iterator[Tuple[str, str]], int]:
    ...
    for root, dirs, files in os.walk(input_dir):
        for file in files:
            if file.endswith('.pod5'):
                ...
                fast5_output_path = os.path.join(output_dir, relative_path, file.replace('.pod5', '.fast5'))
...
def convert_pod5_to_raw_fast5(input_dir: str, output_dir: str, num_processes: int = 4):
    """
    ...
    with WorkerPool(n_jobs=min(num_processes, num_pod5_files)) as pool:
        pool.map(convert_pod5_to_raw_fast5_worker, 
            [(pod5_filepath, fast5_filepath) for pod5_filepath, fast5_filepath in p5_f5_filepaths], 
